...
% Tracking mode.
        ...
        if numPts >= 10
            ...
            height = max(bboxPoints(:, 2)) - min(bboxPoints(:, 2));
...
% --------------------------------------------------------------------------------------------
            ...
            % using the inpolygon function slows down runtime drastically
            % using a raycasting function generated by ChatGPT
            insidePolygon = isPointInsidePolygon(xq(:), yq(:), ellipseVertices(:, 1), ellipseVertices(:, 2));
...
function insideMatrix = isPointInsidePolygon(x, y, polyX, polyY)
